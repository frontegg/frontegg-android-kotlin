...
open class EmbeddedE2ETestCase {
    ...
    /**
     * Wait for text to appear in the WebView via window hierarchy XML dump.
     * This is the ONLY reliable way to detect text inside WebView on CI emulators —
     * the native a11y tree does NOT include WebView DOM content.
     */
    protected fun waitForWebViewText(fragment: String, timeoutMs: Long = 60_000): Boolean {
        val end = System.currentTimeMillis() + timeoutMs
        while (System.currentTimeMillis() < end) {
            if (windowHierarchyContains(fragment)) return true
            Thread.sleep(1_500)
    ...
    /** Attempt password login but expect an error message in the WebView. */
    protected fun loginWithPasswordExpectError(errorFragment: String, timeoutMs: Long = 90_000) {
        ...
        Thread.sleep(5_000)
        val found = waitForWebViewText(errorFragment, timeoutMs)
        if (!found) {
            throw AssertionError("Expected error containing '$errorFragment' but not found in WebView")
        }
    }

    /** Login with magic code flow. Mock server must have magicCode strategy enabled. */
    protected fun loginWithMagicCode(email: String = "test@frontegg.com") {
        waitForDesc("LoginPageRoot", 120_000)
        tapDesc("E2EEmbeddedPasswordButton")
        device.wait(Until.hasObject(By.clazz("android.webkit.WebView")), 30_000)
        Thread.sleep(10_000)
        // Wait for magic code page to render
        waitForWebViewText("verification code", 60_000)
        Thread.sleep(3_000)
        val code = mock.getIssuedMagicCode(email)
            ?: throw AssertionError("No magic code issued for $email")
        // Type the code via Espresso Web
        try {
            authWebView()
                .withElement(findElement(Locator.ID, "e2e-magic-code"))
                .perform(webClick())
            Thread.sleep(500)
            device.findObject(By.focused(true))?.text = code
        } catch (_: Throwable) {
    ...
    protected fun completeMfaChallenge(code: String = "123456") {
        Thread.sleep(8_000)
        device.wait(Until.hasObject(By.clazz("android.webkit.WebView")), 30_000)
        waitForWebViewText("code", 30_000)
        Thread.sleep(3_000)
        try {
            authWebView()
                .withElement(findElement(Locator.ID, "e2e-mfa-code"))
                .perform(webClick())
            Thread.sleep(500)
            device.findObject(By.focused(true))?.text = code
        } catch (_: Throwable) {
    ...
    /** Navigate to signup page via password page "Sign up" link and fill the form. */
    protected fun navigateToSignupAndFill(
        ...
        Thread.sleep(8_000)
        // Fill form via Espresso Web click + UiAutomator text
        try {
